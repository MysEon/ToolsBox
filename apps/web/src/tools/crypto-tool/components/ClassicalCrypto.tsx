...
export default function ClassicalCrypto() {
  ...
  const inputClass = 'w-full px-3 py-2 bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-600 rounded-lg text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:ring-2 focus:ring-amber-500 focus:border-transparent transition-colors';
  ...
  return (
    <div className="space-y-5">
      ...
      {selectedMethodData?.requiresKey && (
        <div>
          <label className="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">{keyLabel}</label>
          <input
            type="text"
            value={key}
      ...
      <div>
        ...
        <textarea
          ...
          placeholder={!isBidirectional ? '请输入要转换的文本...' : `请输入要${operation === 'encrypt' ? '加密' : '解密'}的文本...`}
          rows={4}
      ...
      <div className="flex gap-3">
        ...
        >
          {isProcessing ? '处理中...' : !isBidirectional ? '转换' : operation === 'encrypt' ? '加密' : '解密'}
        </button>
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
              {!isBidirectional ? '转换结果' : operation === 'encrypt' ? '加密结果' : '解密结果'}
            </label>
            <button onClick={handleCopy} className="flex items-center gap-1 text-xs text-amber-600 dark:text-amber-400 hover:text-amber-700 transition-colors">
      ...
      {/* Algorithm info */}
      {selectedMethodData && (
        <div className="p-4 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800 rounded-lg text-sm text-amber-700 dark:text-amber-400">
          <p className="font-medium mb-1">算法说明</p>
          <p>{selectedMethodData.description}</p>
          {selectedMethod === 'morse' && <p className="mt-1 text-amber-600/80 dark:text-amber-400/80">摩斯密码使用空格分隔字符，斜杠(/)表示空格</p>}
          {selectedMethod === 'atbash' && <p className="mt-1 text-amber-600/80 dark:text-amber-400/80">Atbash密码是对称的，加密和解密使用相同的操作</p>}
          {selectedMethod === 'rot13' && <p className="mt-1 text-amber-600/80 dark:text-amber-400/80">ROT13是对称的，对同一文本执行两次ROT13会得到原文</p>}
        </div>
      )}
    </div>
  );
