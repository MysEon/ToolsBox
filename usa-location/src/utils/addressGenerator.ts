...
import { US_STATES, StateInfo } from '../data/states';
...
// 生成真实格式的美国地址
export function generateAddress(stateFilter?: string, cityFilter?: string): GeneratedAddress {
  let selectedState: StateInfo;
  ...
  if (stateFilter) {
    ...
    selectedState = state || faker.helpers.arrayElement(US_STATES);
  } else {
    selectedState = faker.helpers.arrayElement(US_STATES);
  }

  let selectedCity: string;
  if (cityFilter && selectedState.cities.some(city => 
    city.toLowerCase().includes(cityFilter.toLowerCase())
  )) {
    const matchingCities = selectedState.cities.filter(city =>
      city.toLowerCase().includes(cityFilter.toLowerCase())
    );
    selectedCity = faker.helpers.arrayElement(matchingCities);
  } else {
    selectedCity = faker.helpers.arrayElement(selectedState.cities);
  }

  // 生成街道地址
  const streetNumber = faker.number.int({ min: 1, max: 9999 });
  const streetName = faker.person.lastName(); // 使用姓氏作为街道名
  const streetSuffix = faker.helpers.arrayElement([
    'St', 'Ave', 'Blvd', 'Dr', 'Ln', 'Rd', 'Ct', 'Pl', 'Way', 'Pkwy'
  ]);

  // 生成邮编（基于州的真实邮编格式）
  const zipCode = generateZipCode(selectedState.abbreviation);

  // 生成坐标（在州坐标附近的随机位置）
  const coordinates = {
    lat: selectedState.coordinates.lat + (Math.random() - 0.5) * 2, // ±1度范围
    lng: selectedState.coordinates.lng + (Math.random() - 0.5) * 2
  };
...
export function generatePersonalInfo(): PersonalInfo {
  ...
  return {
    ...
    creditCard: {
      ...
      expirationDate: faker.date.future({ years: 5 }).toLocaleDateString('en-US', { 
        year: '2-digit', 
        month: '2-digit' 
      }),
...
// 生成完整档案
export function generateCompleteProfile(stateFilter?: string, cityFilter?: string): CompleteProfile {
  return {
    address: generateAddress(stateFilter, cityFilter),
    personal: generatePersonalInfo()
  };
}

// 生成真实格式的SSN
function generateSSN(): string {
  const area = faker.number.int({ min: 100, max: 899 }); // 避免无效的区域号
...
function generateZipCode(stateAbbr: string): string {
  // 美国各州的邮编前缀范围
  const zipPrefixes: { [key: string]: string[] } = {
    ...
    'MD': ['206', '207', '208', '209', '210', '211', '212', '214', '215', '216', '217', '218', '219', '220', '221'],
  ...
  const prefixes = zipPrefixes[stateAbbr] || ['000'];
  const prefix = faker.helpers.arrayElement(prefixes);
  const suffix = faker.number.int({ min: 10, max: 99 }).toString().padStart(2, '0');
  
  return `${prefix}${suffix}`;
}

// 批量生成
export function generateMultipleProfiles(count: number, stateFilter?: string, cityFilter?: string): CompleteProfile[] {
  return Array.from({ length: count }, () => generateCompleteProfile(stateFilter, cityFilter));
}
