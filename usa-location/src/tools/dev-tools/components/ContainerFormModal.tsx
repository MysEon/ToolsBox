...
export function ContainerFormModal({ container, onClose, onSave }: ContainerFormModalProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) newErrors.name = '容器名称不能为空';
    ...
    if (!formData.serverName.trim()) newErrors.serverName = '服务器名称不能为空';
    if (!formData.containerName.trim()) newErrors.containerName = '容器名称不能为空';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        const updatedContainer: CustomContainer = {
          ...container,
        ...
        ContainerStorage.saveContainer(updatedContainer);
      } else {
        ...
        ContainerStorage.saveContainer(newContainer);
      }
      onSave();
    } catch (error) {
      console.error(isEdit ? '更新容器失败:' : '保存容器失败:', error);
      setErrors({ submit: isEdit ? '更新失败，请重试' : '保存失败，请重试' });
    } finally {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            ...
            <div className="col-span-3">
              ...
              <input type="text" value={formData.url}
                ...
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.url ? 'border-red-300' : 'border-gray-300'}`}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">端口 (可选)</label>
            <input type="number" value={formData.port} min="1" max="65535"
              onChange={(e) => setFormData(prev => ({ ...prev, port: e.target.value }))}
              className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.port ? 'border-red-300' : 'border-gray-300'}`}
              placeholder="8080" />
          ...
          <div>
            ...
            <input type="text" value={formData.serverName}
              onChange={(e) => setFormData(prev => ({ ...prev, serverName: e.target.value }))}
              className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.serverName ? 'border-red-300' : 'border-gray-300'}`}
              placeholder="例如: 阿里云服务器A" />
            {errors.serverName && <p className="text-red-600 text-xs mt-1">{errors.serverName}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Docker容器名称 *</label>
            <input type="text" value={formData.containerName}
              onChange={(e) => setFormData(prev => ({ ...prev, containerName: e.target.value }))}
              className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent ${errors.containerName ? 'border-red-300' : 'border-gray-300'}`}
            ...
            {formData.tags.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {formData.tags.map((tag) => (
                  <span key={tag} className="inline-flex items-center space-x-1 px-2 py-1 bg-blue-100 text-blue-700 text-xs rounded">
                    <span>{tag}</span>
